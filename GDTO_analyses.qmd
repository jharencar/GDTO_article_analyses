...
# 2022
sum_dry_ra_22 <- rain %>% 
   filter(date >= "2021-12-01" & date <= "2022-04-30", chk_note  != "missing") %>%
...
library(ade4) # PCA
library(sp) # Project to Coordinate system
library(raster)
...
# Filter for only species in focal clade:
occurrence_data <- occurrence_data %>% 
  filter(acceptedScientificName %in% c("Costus allenii Maas", "Costus bracteatus Rowlee", "Costus lasius Loes.", "Costus villosissimus Jacq."))
...
for (i in 1:nrow(sis.sum)) {
  plot(df.downsampled$Axis1,df.downsampled$Axis2, xlab="climate axis 1", ylab="climate axis 2", pch=20, col="white",lwd=0)
  ...
  points(sis.d$Axis1,sis.d$Axis2, col="#50be73", pch=16, lwd=0) #lasius green
  points(sis.b$Axis1,sis.b$Axis2, col="#56B4E9", pch=16, lwd=0) #bracteatus blue
  points(sis.a$Axis1,sis.a$Axis2, col="#cc79a7", pch=16, lwd=0) #allenii pink
  points(sis.c$Axis1,sis.c$Axis2, col="#FFD700", pch=16, lwd=0) #vill yellow
  ...
  legend("bottomleft", 
         ...
         col = c("#50be73", "#56B4E9", "#cc79a7",  "#FFD700"), 
         pch = 16, 
         bty = "n",
         pt.cex = 1.5, 
         cex = 1, 
...
# Figure S1 - Flavonid concentrations
ggsave("figures/JoEB_figure_S1.pdf", Fl, device = "pdf", width = 10, height = 8, units = "cm")
ggsave("figures/JoEB_figure_S1.png", Fl, device = "png", width = 10, height = 8, units = "cm")
...
# plotting the preference value instead of raw data
choice <- ggplot(BCT.no.0, aes(x = 1, y = preference)) +
  geom_violin(fill = "gray91", color = "black") +
  geom_point(aes(fill = ifelse(preference > 0, "preference for vill", "preference for alle")),
             position = position_jitter(seed = 1, width = 0.25), size = 2, shape = 21, color = "black") +
  scale_fill_manual(values = c("preference for vill" = "#FFD700", "preference for alle" = "#cc79a7"), labels = c(expression("Preference for\n"~italic("C. villosissimus")),
                               expression("Preference for\n"~italic("C. allenii")))) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  ggtitle("Beetle choice trials") +
  geom_text(x=1.25, y=0.85, aes(label = "p=0.013"), size=3.5) +
...
## Figure 2 - Chemical defense and beetle preference
```{r}
# Joining the plots with patchwork
 P + OC + S + choice + plot_annotation(tag_levels = 'A', tag_suffix = ')') & 
...
ggsave("figures/JoEB_figure_2.png", device = "png", width = 18, height = 19, units = "cm")
ggsave("figures/JoEB_figure_2.pdf", device = "pdf", width = 18, height = 19, units = "cm")
```
...
## Figure 3 - Environmental impacts on herbivory
...
ggsave("figures/Figure_3_Herbivory_patterns.pdf", device = "pdf", width = 24, height = 9, units = "cm")
```
