...
BCT.no.0 <- BCT.no.0 %>% mutate(preference_beta = (preference + 1)/(1+1),
                                preference_beta = case_when(
                                  preference_beta == 0 ~ preference_beta + 0.0001,
                                  preference_beta == 1 ~ preference_beta - 0.0001, 
                                  TRUE ~ preference_beta))

# Check the fit
fit <- fitdist(BCT.no.0$preference_beta, "beta")
# Check the goodness of fit
descdist(BCT.no.0$preference_beta, "beta", fit$estimate)
plot(fit) # not terrible, the best I can do
...
wilcox.test(BCT.no.0$preference, mu = 0)
```
...
```{r}
#  if the data are highly non-normal, a 1-sample Wilcoxon test: wilcox.test(normalized_diff, mu = 0)
```
